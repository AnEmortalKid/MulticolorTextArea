 ...
 * <p>
 * <strong>Disclaimer:</strong> I copy pasted the TextArea and made minor
 * modifications. When I tried to extend it, I ran into issues with the line
 * wrapping. I did not find a good way to set the cursor at a certain position
 * without implementing my own, therefore I reproduced this one verbatim and
 * just added protected modifiers on some variables that were package access
 * only.
 * </p>
 */
public class ExtensibleTextArea extends TextField {
	...
	/**
	 * Variable to maintain the x offset of the cursor when moving up and down.
	 * If it's set to -1, the offset is reset
	 **/
	...
	public float getPrefHeight() {
		if (prefRows <= 0) {
			...
			if (getStyle().background != null) {
				prefHeight = Math.max(
						prefHeight + getStyle().background.getBottomHeight()
								+ getStyle().background.getTopHeight(),
			...
			if (moveOffset < 0) {
				moveOffset = linesBreak.size <= cursorLine * 2 ? 0
						: glyphPositions.get(cursor)
								- glyphPositions.get(linesBreak
										.get(cursorLine * 2));
			}
			cursorLine = line;
			cursor = cursorLine * 2 >= linesBreak.size ? text.length()
					: linesBreak.get(cursorLine * 2);
			while (cursor < text.length()
					&& cursor <= linesBreak.get(cursorLine * 2 + 1) - 1
					&& glyphPositions.get(cursor)
							- glyphPositions
									.get(linesBreak.get(cursorLine * 2)) < moveOffset) {
				cursor++;
			}
			showCursor();
		}
	...
	protected void updateCurrentLine() {
		...
		int line = index / 2;
		// Special case when cursor moves to the beginning of the line from the
		// end of another and a word
		// wider than the box
		if (index % 2 == 0 || index + 1 >= linesBreak.size
				|| cursor != linesBreak.items[index]
				|| linesBreak.items[index + 1] != linesBreak.items[index]) {
			if (line < linesBreak.size / 2 || text.length() == 0
					|| text.charAt(text.length() - 1) == ENTER_ANDROID
	...
	protected void sizeChanged() {
		// The number of lines showed must be updated whenever the height is
		// updated
	...
	protected void drawSelection(Drawable selection, Batch batch,
		...
		while (i + 1 < linesBreak.size
				&& i < (firstLineShowing + linesShowing) * 2) {
			...
			if (!((minIndex < lineStart && minIndex < lineEnd
				...
				float selectionX = glyphPositions.get(start)
						- glyphPositions.get(linesBreak.get(i));
				float selectionWidth = glyphPositions.get(end)
						- glyphPositions.get(start);

				selection.draw(batch, x + selectionX,
						y - textHeight - font.getDescent() - offsetY,
						selectionWidth, font.getLineHeight());
			}
	...
	protected void drawText(Batch batch, BitmapFont font, float x, float y) {
		...
		for (int i = firstLineShowing * 2; i < (firstLineShowing + linesShowing) * 2
			...
			font.draw(batch, displayText, x, y + offsetY, linesBreak.items[i],
	...
	protected void drawCursor(Drawable cursorPatch, Batch batch,
			BitmapFont font, float x, float y) {
		float textOffset = cursor >= glyphPositions.size
				|| cursorLine * 2 >= linesBreak.size ? 0 : glyphPositions
				.get(cursor)
				- glyphPositions.get(linesBreak.items[cursorLine * 2]);
		cursorPatch.draw(batch, x + textOffset, y - font.getDescent() / 2
	...
	protected void calculateOffsets() {
		...
		if (!this.text.equals(lastText)) {
			...
			float maxWidthLine = this.getWidth()
					- (getStyle().background != null ? getStyle().background
							.getLeftWidth()
							+ getStyle().background.getRightWidth() : 0);
			linesBreak.clear();
			int lineStart = 0;
			int lastSpace = 0;
			char lastCharacter;
			for (int i = 0; i < text.length(); i++) {
				...
				if (lastCharacter == ENTER_DESKTOP
						|| lastCharacter == ENTER_ANDROID) {
	...
	protected void moveCursor(boolean forward, boolean jump) {
		...
		if (index >= 0 && index + 1 < linesBreak.size
				&& linesBreak.items[index] == cursor
	...
	public class TextAreaListener extends TextFieldClickListener {
		...
		protected void setCursorPosition(float x, float y) {
			...
			cursorLine = (int) Math.floor((height - y) / font.getLineHeight())
					+ firstLineShowing;
		...
		public boolean keyDown(InputEvent event, int keycode) {
			...
			if (stage != null
					&& stage.getKeyboardFocus() == ExtensibleTextArea.this) {
